...
const calculateTotalScore = (entry: any): number => {
  ...
  if (entry.scores?.criteria?.length > 0) {
    const total = entry.scores.criteria.reduce((sum: number, criterion: any) => {
      if (criterion.final_score) {
        // Extract number before the '/' → e.g. "1.6666666667/2" → 1.6666666667
        const numericPart = parseFloat(String(criterion.final_score).split("/")[0]);
...
const transformApiData = (entries: any[], scoreSetSlug: string): LeaderboardEntry[] => {
  if (!entries) return [];
 ...
 */
export async function syncLeaderboard(config: { apiKey: string }, scoreSetSlug: string) {
    ...
    while (hasMorePages) {
        ...
        const response = await apiRequest(`/leaderboard?${query.toString()}`, config.apiKey);
...
export async function getAllLeaderboardDataForAnalytics(
  ...
  municipalityFilter: string
    ...
    const shouldFilterByMunicipality = municipalityFilter && municipalityFilter !== 'all';
    ...
    try {
        ...
        const dataResult = await sql.query<AnalyticsLeaderboardEntry & { total_score: string }>(`
        ...
        const transformedData: AnalyticsLeaderboardEntry[] = dataResult.rows.map((row: any) => ({
            slug: row.slug,
...
export async function getLeaderboardPage(
  ...
  maxScore: number | undefined,
  municipalityFilter: string
): Promise<{ data: LeaderboardEntry[]; current_page: number; last_page: number; total: number }> {
    if (!scoreSetSlug) {
    ...
    const transformedData: LeaderboardEntry[] = dataResult.rows.map((row: any) => ({
        slug: row.slug,
