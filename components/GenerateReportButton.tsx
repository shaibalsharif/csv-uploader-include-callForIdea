...
export function GenerateReportButton({
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            const reportDataJson = await generatePDFReport({ filteredApps, municipalityFilter, lastSyncTime, scoreSetName, minScore, maxScore });
            ...
            const drawHorizontalBarChart = (title: string, data: { label: string; value: number }[]): void => {
                ...
                data.forEach((item, index) => {
                    ...
                    const labelLines = doc.splitTextToSize(item.label, labelAreaWidth - 2);
                    doc.text(labelLines, margin, currentY + barHeight / 2, { verticalAlign: 'middle' });
                    doc.setFillColor(79, 132, 196).rect(margin + labelAreaWidth, currentY, item.value * scale, barHeight, 'F');
                    doc.text(item.value.toString(), margin + labelAreaWidth + (item.value * scale) + 2, currentY + barHeight / 2, { verticalAlign: 'middle' });
                });
            ...
            const addSection = (title: string, tableData: (string | number)[][], head: string[][], chartData: { label: string; value: number }[]) => {
                ...
                doc.setFontSize(14).setFont('helvetica', 'bold').text(title, 15, yPosition);
                yPosition += 8;
                autoTable(doc, { startY: yPosition, head, body: tableData, theme: 'grid', headStyles: { fillColor: [41, 128, 185] } });
                yPosition = (doc as any).lastAutoTable.finalY + 10;
                if (chartData.length > 0) {
                    drawHorizontalBarChart(`${title.split('. ')[1]} - Chart`, chartData);
                }
            };
            ...
            addSection('2. Municipality Distribution', getTableData(reportData.municipalBreakdown), [['Municipality', 'Count', 'Percentage']], transformForChart(reportData.municipalBreakdown));
            addSection('3. Category (Digital/Non-Digital)', getTableData(reportData.categoryBreakdown), [['Category', 'Count', 'Percentage']], transformForChart(reportData.categoryBreakdown));
            addSection('4. Gender Distribution', getTableData(reportData.genderBreakdown), [['Gender', 'Count', 'Percentage']], transformForChart(reportData.genderBreakdown));
